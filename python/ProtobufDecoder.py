# fork from https://github.com/dannyhann/protobuf_decoder
#
#   python protobufdecoder.py '0A09ED858CEC8AA4ED8AB8'
#   >> [{'field': 1, 'wire_type': 'string', 'data': '테스트'}]
#   python protobufdecoder.py 1a 03 08 96 01
#   >> [{'field': 3, 'wire_type': 'length_delimited', 'data': [{'field': 1, 'wire_type': 'varint', 'data': 150}]}]
#
...
class ParsedResult:
    ...
    def to_dict(self):
        ...
        return dict(field=self.field, wire_type=self.wire_type, data=data)
...
class ParsedResults:
    ...
    def to_dict(self):
        return [sub_result.to_dict() for sub_result in self.results]
...
class Parser:
    ...
    def parse_varint_handler(self, chunk):
        ...
        if self.has_next(chunk):
            ...
            self.parsed_data.append(
                ParsedResult(field=self.target_field, wire_type="varint", data=bit_value))
    ...
    def get_delimited_data_handler(self, chunk):
        ...
        if self.fetcher.has_next:
            ...
            self.parsed_data.append(
                ParsedResult(field=self.target_field, wire_type=wire_type, data=data))
            self.buffer.flush()
            self.fetcher.seek()
            self.state = State.FIND_FIELD
...
if __name__ == '__main__':
    ...
    test_target = ''.join(argv[1:])
    parsed_data = Parser().parse(test_target)
    dct = parsed_data.to_dict()

    #jsn = json.dumps(dct, indent=2, ensure_ascii=False)
    print(dct)
